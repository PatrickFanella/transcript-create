...
- **Private beta:** use the ordered [deployment runbook](../deployment/private-beta.md). Its staging restore is a separate-host, empty-storage WAL-G base-backup plus WAL-replay rehearsal; a logical dump alone is insufficient. The operator must retain ingress negative-probe, health/search-lag/lease/backup/alert, and CUDA smoke evidence.
