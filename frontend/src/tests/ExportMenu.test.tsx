...
describe('ExportMenu', () => {
  ...
  it('renders export button', () => {
    render(<ExportMenu videoId={mockVideoId} isPro={true} />);
  ...
  it('allows non-PDF download when user is not Pro', async () => {
    const user = userEvent.setup();
    render(<ExportMenu videoId={mockVideoId} isPro={false} />);
  ...
  it('shows per-section export info', async () => {
    ...
    expect(screen.getByText(/Use the inline copy link next to any segment/i)).toBeInTheDocument();
  });
});
