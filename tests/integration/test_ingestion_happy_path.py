...
class TestSingleVideoIngestion:
    ...
    def test_fetch_youtube_captions_success(
        self, mock_urlopen, mock_yt_dlp, mock_yt_dlp_single_video, mock_json3_captions
    ):
...
class TestChannelIngestion:
    ...
    def test_channel_metadata_extraction(self, mock_run_json, mock_yt_dlp_channel_playlist):
        ...
        command = mock_run_json.call_args.args[0]
        assert command[0] == "-J"
        assert "yt-dlp" not in command
        assert command[-1] == "https://youtube.com/channel/test_channel_123"
...
class TestJobStateTransitions:
    ...
    def test_job_creation_state(self, db_session):
        ...
        # Verify job state
        result = db_session.execute(text("SELECT state FROM jobs WHERE id = :id"), {"id": job_id}).fetchone()
    ...
    def test_video_creation_after_expansion(self, db_session):
        ...
        # Verify video exists
        result = db_session.execute(text("SELECT state FROM videos WHERE id = :id"), {"id": video_id}).fetchone()
    ...
    def test_video_state_progression(self, db_session):
        ...
        for state in states:
            db_session.execute(
                text("UPDATE videos SET state = :state WHERE id = :id"), {"state": state, "id": video_id}
            )
            db_session.commit()
...
class TestCaptionAvailabilityDetection:
    ...
    def test_detects_json3_captions(self, mock_yt_dlp):
        ...
        mock_data = {"automatic_captions": {"en": [{"ext": "json3", "url": "https://example.com/captions.json3"}]}}
        mock_yt_dlp.return_value = mock_data
    ...
    def test_detects_vtt_captions(self, mock_yt_dlp):
        ...
        mock_data = {"automatic_captions": {"en": [{"ext": "vtt", "url": "https://example.com/captions.vtt"}]}}
        mock_yt_dlp.return_value = mock_data
...
class TestIntegrationWorkflow:
    ...
    @patch("worker.youtube_captions._yt_dlp_json")
    @patch("worker.youtube_captions.urlopen")
    def test_complete_single_video_workflow(
        self, mock_urlopen, mock_yt_dlp, mock_yt_dlp_single_video, mock_json3_captions
    ):
    ...
    def test_job_completion_updates_state(self, db_session):
        ...
        video_result = db_session.execute(
            text("SELECT state FROM videos WHERE job_id = :job_id"), {"job_id": job_id}
        ).fetchone()
